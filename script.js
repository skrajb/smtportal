...
let qIndex = 0; const quoteBox = document.getElementById("quoteBox");
  
function showQuote() { quoteBox.style.opacity = 0; setTimeout(() => {
  
  quoteBox.innerHTML = quotes[qIndex]; quoteBox.style.opacity = 1;
  
  qIndex = (qIndex + 1) % quotes.length; }, 800); }
...
function verifyCredentials() {
  ...
  if(username.value==="smt" && password.value==="1995"){
  
    sessionStorage.setItem('isLoggedIn','true');
  
    isLoggedIn = true; loginModal.style.display='none';
  
    exerciseContainer.classList.remove('hidden');
  
    logoutButton.classList.remove('hidden'); loginBtn.classList.add('hidden');
  
  } else loginMessage.innerText="Invalid credentials (try blank for demo)";
...
function logout(){
  ...
  isLoggedIn=false; exerciseContainer.classList.add('hidden');
  
  logoutButton.classList.add('hidden'); loginBtn.classList.remove('hidden');
  
}
  
function addExercise(lang){
  
  let title=lang==="English"?exerciseTitleEnglish.value:exerciseTitlePunjabi.value;
  
  let text=lang==="English"?exerciseTextEnglish.value:exerciseTextPunjabi.value;
...
function renderExercises(lang) {
  const box = lang === "English" ? exerciseListEnglish : exerciseListPunjabi;
...
function deleteAllExercises(){
  ...
  localStorage.clear(); renderExercises("English"); renderExercises("Punjabi");
  
}
  
window.onload=()=>{ if(isLoggedIn){exerciseContainer.classList.remove('hidden');logoutButton.classList.remove('hidden');loginBtn.classList.add('hidden');}
  
  renderExercises("English");renderExercises("Punjabi"); };async function loadExercisesFromUrl() {
...
const GOOGLE_APP_URL = "https://script.google.com/macros/s/AKfycbwu-7NV-bYnBnejBPMUaGpr8ukTWwcMe5QE8AmNYDhnGGUEWU3u1YxslFg24MyKXq5EUg/exec";
...
function verifyCredentials() {
  // username and password are IDs in the DOM, so they become globals on most browsers,
  // but we'll use getElementById to be explicit and robust.
  ...
  if (u === "smt" && p === "1995") {
    ...
    // register login with backend so this session becomes the active one
    registerLogin(u);

    // start/reset inactivity timer
    resetInactivityTimer();
  } else {
    if (loginMessage) loginMessage.innerText = "Invalid credentials (try blank for demo)";
  }
}
